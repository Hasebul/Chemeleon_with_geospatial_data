...
from demos import prompt_policy, prompt_kr, prompt_sg, prompt_qg, prompt_gm, prompt_context_gen
...
class solver:
    ...
    def google_maps(self):
        ...
        start_index = information.find('{')
        end_index = information.rfind('}')
        information = information[start_index:end_index + 1].lstrip()
        ...
        origin = geocode(information['origin'])
        destination = geocode(information['destination'])
        alternatives = True if information['alternative'] == 'True' or information['alternative'] == 'true' or information['alternative'] == 'TRUE' else False
        waypoints = None if information['waypoints']=="None" or information['waypoints']=="none" else geocode(information["waypoints"])
        # now call the Google Maps api and fetch information documentation
        extract_information = directions(origin=origin, destination=destination, mode=information["mode"],waypoints=waypoints,alternatives=alternatives)
        ...
        # Write code for context generation
        # get the example
        question_text = self.get_question_text()
        metadata = self.get_metadata()
        # response = self.cache["response"] if "response" in self.cache else ""
        ...
        # demo prompt
        # demo_prompt = prompt_gm.prompt_1.strip()
        demo_prompt = prompt_context_gen.prompt_1.strip()
        # full prompt
        full_prompt = demo_prompt + "\n\n" + json.dumps(extract_information)
        messages = [
            {"role": "user", "content": full_prompt},
        ]

        # find the input of Google Maps
        context = get_chat_response(messages, self.api_key, self.qg_engine, self.qg_temperature,
                                        self.qg_max_tokens)
        # update the cache
        self.cache["response"] = context
        self.cache["query_generator:input"] = test_prompt
        self.cache["query_generator:output"] = context
        return test_prompt, context
